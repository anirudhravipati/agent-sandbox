...
# gsandbox - Hardened sandbox for running Gemini CLI
...
show_help() {
    cat << EOF
${SCRIPT_NAME} - Run Gemini CLI in a hardened sandbox using bubblewrap

DESCRIPTION:
    This script creates an isolated environment for running Gemini CLI with
    ...
    • Allows read-write access only to Gemini configs and current directory
    • Logs all session activity for review

USAGE:
    ${SCRIPT_NAME} [OPTIONS] [-- GEMINI_ARGS...]

OPTIONS:
    -h, --help              Show this help message and exit
    -v, --version           Show version information and exit
    --install               Install to ~/.local/bin/gsandbox
    --uninstall             Remove from ~/.local/bin/gsandbox
...
GEMINI ARGUMENTS:
    All Gemini CLI options are supported. Use '--' to separate sandbox
    options from Gemini options:

        ${SCRIPT_NAME} [SANDBOX_OPTIONS] -- [GEMINI_OPTIONS]

    Common Gemini options:
        -m, --model <model>   Select model
        -p, --prompt <msg>    Run in non-interactive mode with a prompt
        -r, --resume          Resume previous conversation

    Run 'gemini --help' for the full list of Gemini options.

EXAMPLES:
    ${SCRIPT_NAME}                    Run Gemini in sandbox with default settings (YOLO mode)
    ...
    ${SCRIPT_NAME} -- -m flash        Pass arguments to Gemini
...
READ-ONLY DIRECTORIES:
    ...
    the -R/--ro-dir flag or a .gsandbox.ro file in the working directory.

    The .gsandbox.ro file lists one directory per line (relative to the
    ...
    Entries from .gsandbox.ro are combined with any -R/--ro-dir flags.
...
SESSION LOGS:
    ...
    Logs are saved to: gemini_session_YYYY-MM-DD_HH-MM-SS.log
...
INSTALL_PATH="$INSTALL_DIR/gsandbox"
...
do_install() {
    ...
    echo "✅ Installed! You can now run 'gsandbox' from anywhere."
}
...
do_uninstall() {
    if [ -f "$INSTALL_PATH" ]; then
        ...
        echo "gsandbox is not installed at $INSTALL_PATH"
        exit 1
    fi
}
...
if [ "$SAFE_MODE" = true ]; then
    ...
    echo "⚡ Permission Mode: YOLO (--yolo)"
fi
...
# Layer 2: Unlock specific Gemini config files (Read-Write)
for config_path in "$HOME_DIR"/.gemini*; do
...
RO_FILE="$CURRENT_DIR/.gsandbox.ro"
if [ -f "$RO_FILE" ]; then
    RO_FILE_COUNT=0
    ...
    echo "📄 Loaded .gsandbox.ro ($RO_FILE_COUNT entries)"
fi
...
if [ "$SAFE_MODE" = true ]; then
    FULL_CMD="bwrap $BWRAP_ARGS --share-net --new-session --die-with-parent gemini ${GEMINI_ARGS[*]}"
else
    FULL_CMD="bwrap $BWRAP_ARGS --share-net --new-session --die-with-parent gemini --yolo ${GEMINI_ARGS[*]}"
fi
